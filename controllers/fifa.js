...
const playersModel = require("../models/fifa");
...
const listPlayers = async (req = request, res = response) => {
  ...
  //declaracion de variable para conexión a la base de datos
  let conn;
  ...
  PAGINACION   
   ...
   limit= es el limite o la cantidad de datos que quieres que muestre
   offset= es el desplazamiento o cuantos registros saltar antes de empezar a mostrar los resultados.
   parseInt= se asegura que las varibles limit y offset reciban solo numeros
...
lo almacena en la variable player
    */
    ...
    //en formato JSON
    res.json({ msg: "Obtained result", player });
  } catch (error) {
    console.log(error);
    res.status(500).json(error);
  } finally {
...
//####################### SEARCH BY NAME ################################
const listPlayersByName = async (req = request, res = response) => {
  
  // Extraer el nombre del jugador desde los parámetros de la URL.
  const { short_name } = req.query;

  // Verificar si el nombre del jugador es válido.
  if (!short_name) {
    res.status(400).json({ msg: "--Invalid parameter--" }); // Enviar respuesta de error 400 si no hay nombre de jugador.
    return;
  }
  ...
  try {
    ...
    const players = await conn.query(playersModel.getByword, [short_name]);

    // Verificar si se encontraron jugadores.
    if (!players || players.length === 0) {
      res.status(404).json({ msg:  `No Player found for the given name: ${short_name}`  }); // Enviar respuesta de error 404 si no se encontraron jugadores.
      return;
    }
    const rowCount = players.length;
...
const addPlayer = async (req = request, res = response) => {
  ...
  try {
    ...
    const [existingPlayer] = await conn.query(playersModel.getByID, [id]);
    ...
    const [existingData] = await conn.query(
      ...
      [short_name],
      playersModel.getByFull_name,
      [Full_name],
      playersModel.getByclub_jersey_number,
      [club_jersey_number]
    );
    if (existingData) {
      ...
      res
        .status(409)
        .json({
          msg: `The following fields cannot be repeated:short_name, full name: ${Full_name} , jersey number: ${club_jersey_number}`,
        });
      // Finaliza la ejecución de la función
      return;
    }
    ...
    res.json({ msg: "Player added successfully",player });
  } catch (error) {
...
// exportamos la función como middleware
//########################### UPDATE ########################
const updatePlayer = async (req = request, res = response) => {
  ...
  try {
    ...
    if (short_name === playerExists.short_name) {
      res
        .status(409)
        .json({ msg: `Short name '${short_name}' already exists` });
      return;
    }
    ...
    const playerUpdated = await conn.query(
      playersModel.updateUser,
      [...player, id],
      (err) => {
        if (err) throw err;
      }
...
const deteleUsers = async (req = request, res = response) => {
  ...
  try {
    ...
    if (!playerExists || playerExists.id === 0) {
      res.status(404).json({ msg: "Player not found" });
      return;
    }
